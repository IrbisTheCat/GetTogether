...
isFreeform=function(){'use strict';
return !!this.info.freeform;};
i.prototype.

setSelected=function(j){'use strict';
...
setElement=function(j){'use strict';
c('DataStore').set(j,'Token',this);
this.element=j;
return this;};
i.prototype.

isRemovable=function(){'use strict';
return c('CSS').hasClass(this.element,'removable');};
i.prototype.

getTextDirection=function(){'use strict';
...
createElement=function(j,k){'use strict';
var l=this.paramName,
m=this.getValue(),
n=this.getText(),
o=c('DOM').create('span',{className:'uiTokenText'},n),

p=null;
if(this.info.removable!==false)
p=c('DOM').create('a',
{href:'#',
'aria-label':h._(["Remove {item}","77483de97c07d982ae76a79d0396e1ea"],
[h.param('item',n)]),
...
if(j)
c('CSS').addClass(p,'uiCloseButtonSmallGray');
...
s={className:'removable uiToken'},
t=this.getTextDirection();
if(t!==null)
s.dir=t;


var u=c('DOM').create('span',
s,
[o,q,r,p]);

if(j)
c('CSS').addClass(u,'uiTokenGray');



if(k){
var v=c('DOM').create('i',{className:k});
...
/** js/modules/WeakToken.js */




__d('WeakToken',['CSS','Token'],(function a(b,c,d,e,f,g){var h,i;h=babelHelpers.inherits
...
(j,c('Token'));i=h&&h.prototype;j.prototype.
...
/** js/modules/Tokenizer.js */





__d('Tokenizer',['Arbiter','ArbiterMixin','CSS','DataStore','DOM','DOMQuery','Event','Focus','Input','Keys','Parent','StickyPlaceholderInput','Style','TextMetrics','Token','UserAgent_DEPRECATED','WeakToken','createObjectFrom','emptyFunction','mixin'],(function a(b,c,d,e,f,g){var h,i,
...
init=function(l,m,n,o){'use strict';
...
this.unique={};
if(l){
this.populate(l);}else 

c('DOM').empty(this.tokenarea);

this.updateTokenarea();};
k.prototype.

populate=function(l){'use strict';
var m=[];
...
setTokenarea=function(l){'use strict';
var m=!this.tokenarea;
if(l!==this.tokenarea){
if(this.tokenarea){
c('DOM').remove(this.tokenarea);
for(var n in this._tokenareaListeners)
this._tokenareaListeners[n].remove();}


this._tokenareaListeners=c('Event').listen
(l,
...
initBehaviors=function(){'use strict';
...
var m=window.TokenizerBehaviors&&
window.TokenizerBehaviors[l];
...
initTypeahead=function(){'use strict';
...
this.typeahead.subscribe('select',function(n,o){return this.handleSelect(o);}.bind(this));
this.typeahead.subscribe('blur',this.handleBlur.bind(this));};
k.prototype.

handleBlur=function(event){'use strict';
this.inform('blur',{event:event});
this.updatePlaceholder();};
k.prototype.

handleSelect=function(l){'use strict';
var m=l.selected;
if('uid' in m){
this.updateInput();
...
if(!m)return;
if(event.type!='keydown'||c('Event').getKeyCode(event)==c('Keys').RETURN)
this.processEvents(event,l,m);};
...
focusOnTokenRemoval=function(event,l){'use strict';
...
paste=function(event){'use strict';
...
updateInput=function(l){'use strict';
...
setPlaceholder=function(l){'use strict';
this.placeholder=l;
if(this.stickyPlaceholder)
c('StickyPlaceholderInput').setPlaceholderText(this.input,l);

this.updatePlaceholder();};
k.prototype.

updatePlaceholder=function(){'use strict';
...
adjustWidth=function(l){'use strict';
...
if(!l&&this.input.value==='')
l=this.placeholder;


var m=j;
if(l!==this.placeholder||
...
getToken=function(l){'use strict';
return this.unique[l]||null;};
k.prototype.

getTokens=function(){'use strict';
return this.tokens||[];};
k.prototype.
...
getTokenFromElement=function(l){'use strict';
return c('DataStore').get(l,'Token');};
k.prototype.

getTokenValues=function(){'use strict';
if(!this.tokens)
return [];
...
getFirstToken=function(){'use strict';
return this.tokens[0]||null;};
k.prototype.

getLastToken=function(){'use strict';
return this.tokens[this.tokens.length-1]||null;};
k.prototype.

hasMaxTokens=function(){'use strict';
...
if(!n)
n=l.weak_reference?
new (c('WeakToken'))(l,this.paramName):
new (c('Token'))(l,this.paramName);
...
addToken=function(l){'use strict';
...
removeAllTokens=function(){'use strict';
this.reset();
this.inform('changeTokens');
this.inform('removeAllTokens');};
k.prototype.

updateTokenarea=function(){'use strict';
var l=this.typeahead.getCore(),
...
_tokenKey=function(l){'use strict';
return l.uid+(l.freeform?':':'');};
k.prototype.

_getWidthOffset=function(){'use strict';
if(this._widthOffset===null){
var l=this.input.clientWidth,
m=c('Style').getFloat(this.input,'width');

if(l==m){
this._widthOffset=c('Style').getFloat(this.input,'paddingLeft')+
c('Style').getFloat(this.input,'paddingRight');}else 
...
_getMetrics=function(){'use strict';
if(!this._metrics)
this._metrics=new (c('TextMetrics'))(this.input,this.inline);

return this._metrics;};
k.prototype.

_getIsInDOM=function(){'use strict';
return this._isInDOM||
(this._isInDOM=c('DOMQuery').contains(document.body,this.input));};
k.
...
/** js/plugins/PluginUITypeahead.js */
...
function h(){
return c('DOMQuery').scry(document.body,'.uiTokenizer').map
(function(j){
...
return {x:m.x+n.width,y:m.y+n.height};});}
...
{width:function j(){
return Math.max.apply(null,h().map(function(k){return k.x;}));},


height:function j(){
return Math.max.apply(null,h().map(function(k){return k.y;}));}};
...
/** js/plugins/PluginSendFlyout.js */
...
m.setAttribute('aria-expanded','false');}else 
...
(p.offsetWidth,
j.offsetWidth,
c('PluginUITypeahead').width());},


function(){
return Math.max
(p.offsetHeight,
j.offsetHeight+j.offsetTop,
c('PluginUITypeahead').height());},
