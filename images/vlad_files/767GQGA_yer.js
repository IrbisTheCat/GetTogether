if (self.CavalryLogger) { CavalryLogger.start_js(["RZozx"]); }

/** www/__virtual__/x/XBirthdayEventCreatePromptXOutController.js */



__d("XBirthdayEventCreatePromptXOutController",["XController"],(function a(b,c,d,e,f,g){
f.exports=c("XController").create("\/birthdays\/event_create_prompt_xout\/",{});}),

null);

/** js/events/creation_prompts/BirthdayEventPromptXOutController.js */





__d('BirthdayEventPromptXOutController',['AsyncRequest','CSS','Event','XBirthdayEventCreatePromptXOutController'],(function a(b,c,d,e,f,g){
...
new (c('AsyncRequest'))
(c('XBirthdayEventCreatePromptXOutController').getURIBuilder().
getURI()).
...
/** www/__virtual__/x/XFamilyMentionsNUXDialogController.js */



__d("XFamilyMentionsNUXDialogController",["XController"],(function a(b,c,d,e,f,g){
f.exports=c("XController").create("\/family_mentions\/nux_dialog\/",{__asyncDialog:{type:"Int"}});}),

null);

/** js/search/typeahead/FamilyRelationshipMentions.js */
...
if(c('FamilyMentionsData').hasAcceptedNUX||
j)
return false;
...
new (c('AsyncRequest'))().
setURI('/ajax/family_mentions/deny_nux').
send();
l(k);});
...
j=true;
new (c('AsyncRequest'))().
setURI('/ajax/family_mentions/confirm_nux').
send();
l(k);
n.finishHide();});});};
...
var o=l.results[n],
p=
this._getNameToDisplay(o,m,j);
if(p!==null){
o.text=p;
if(Object.prototype.hasOwnProperty.call(o,'default_name')&&
o.text!==o.default_name){
o.subtext=o.default_name;}else 
...
_getNameToDisplay=function(i,j,k){'use strict';


if(Object.prototype.hasOwnProperty.call(i,'default_name')&&
c('TokenizeUtil').isQueryMatch(j,i.default_name))
return i.default_name;
...
var o=m[n];
if(c('TokenizeUtil').isQueryMatch(j,o))
return o;}}
...
_getAvailableAlternateNameFields=function(){'use strict';
var i=['alternate_names'];
if(c('FamilyMentionsData').allowFamilyNames)
i.push('family_names');
...
/** js/ui/core/ui/form/mentions/MentionsInputMatchers.js */
...
function m(p,q){'use strict';
this.$Matchers_triggers=p;
this.$Matchers_punc=q;
this.$Matchers_constructMatchers();}
m.prototype.

$Matchers_constructMatchers=function(){'use strict';
var p=this.$Matchers_constructName(this.$Matchers_punc),
q=this.$Matchers_constructToken
(this.$Matchers_triggers,
this.$Matchers_punc),

r=this.$Matchers_constructMainMatcher
(this.$Matchers_triggers,
q),

s=this.$Matchers_constructAutoMatcher
(p,
r),

t=this.$Matchers_constructUserMatcher(p);

this.$Matchers_triggerMatcher=new RegExp('['+this.$Matchers_triggers+']$');
this.$Matchers_mainMatcher=new RegExp(r+'$');
this.$Matchers_userMatcher=new RegExp(t+'$');
this.$Matchers_autoMatcher=new RegExp(s+'$');
this.$Matchers_hashtagMatcher=new RegExp('['+i+']');};
m.prototype.

$Matchers_constructName=function(p){'use strict';
...
$Matchers_constructToken=function(p,q){'use strict';
return '(?:[^'+p+q+']|['+q+'][^ '+q+'])';};
m.prototype.


$Matchers_constructMainMatcher=function(p,q){'use strict';
return '(?:^|\\s)(['+p+']('+q+'{0,20}))';};
m.prototype.



$Matchers_constructAutoMatcher=function(p,q){'use strict';
return '(?:(?:^|[^#])('+p+'+)|'+q+')';};
m.prototype.


$Matchers_constructUserMatcher=function(p){'use strict';
return '(?:'+p+'{4,})';};
m.prototype.

getMainMatcher=function(){'use strict';
return this.$Matchers_mainMatcher;};
m.prototype.

getTriggerMatcher=function(){'use strict';
return this.$Matchers_triggerMatcher;};
m.prototype.

getAutoMatcher=function(){'use strict';
return this.$Matchers_autoMatcher;};
m.prototype.

getHashtagMatcher=function(){'use strict';
return this.$Matchers_hashtagMatcher;};
...
/** js/ui/core/ui/form/mentions/MentionsInput.js */
...
__d('MentionsInput',['cx','Arbiter','ArbiterMixin','Bootloader','CSS','DataStore','DOM','Event','FbtResult','FlipDirection','Input','InputSelection','Keys','MentionsInputMatchers','Parent','Style','TokenizeUtil','UserAgent_DEPRECATED','htmlize','mixin','removeFromArray'],(function a(b,c,d,e,f,g,h){var i,j,
...
l=new RegExp(k,'g'),
m=function v(w){return w+k;},


n=function v(w){return w.replace
(/(@+)(\[[0-9]+\:([^\]]|\\\])*\]+)/g,
'$1 $2');},
...
(u,c('mixin')(c('ArbiterMixin')));j=i&&i.prototype;
function u(v,w,x,y,z,aa){'use strict';
...
try{ba=document.activeElement===this._input;}catch(da){}
if(ba){
setTimeout(ca,0);}else 

var ea=c('Event').listen(this._input,'focus',function(){
setTimeout(ca,0);
ea.remove();});
...
init=function(v,w,x){'use strict';
...
c('Input').setValue(this._input,v.value);
...
c('FlipDirection').setDirection(this._input);
...
_getMarkedValue=function(){'use strict';
...
getRawValue=function(){'use strict';
this._update();
return c('Input').getValue(this._hiddenInput);};
u.prototype.

checkValue=function(){'use strict';
var v=this._typeahead.getCore().getValue();
if(this._matchers.getTriggerMatcher().exec(v)||v==='')
this.inform('sessionEnd',{});};

u.prototype.

getTypeahead=function(){'use strict';
return this._typeahead;};
u.prototype.

hasChanges=function(){'use strict';
return !c('Input').isEmpty(this._input)&&
...
_initTypeahead=function(){'use strict';
...
_handleBlur=function(){'use strict';
...
_handleKeydown=function(event){'use strict';


var v=event.keyCode;
if(v==c('Keys').BACKSPACE||v==c('Keys').DELETE)
this._handleBackspaceAndDelete(event,v);

if(v==c('Keys').LEFT||v==c('Keys').RIGHT)
setTimeout(this._handleLeftAndRight.bind(this,v),10);};

u.prototype.

_handleLeftAndRight=function(v){'use strict';
...
var ca=aa?-1:1;
if(w.charAt(y)==k)
c('InputSelection').set(this._input,y+ca);}else

if(aa&&w.charAt(y)==k){
c('InputSelection').set(this._input,y-1,z);}else
if(aa&&w.charAt(z)==k){
c('InputSelection').set(this._input,y,z-1);}else
if(ba&&w.charAt(z)==k){
c('InputSelection').set(this._input,y,z+1);}else
if(ba&&w.charAt(y)==k)
...
_handleBackspaceAndDelete=function(event,v){'use strict';
...
if(this._offsetIsInsideMention(w.start+1)&&
this._offsetIsInsideMention(w.end)){
x=v===c('Keys').BACKSPACE;}else 
...
_offsetIsInsideMention=function(v){'use strict';
for(var w=0;w<this._offsets.length;w++)
if(v>this._offsets[w][0]&&v<=this._offsets[w][1])
return true;
...
_filterResults=function(v){'use strict';
...
if(this._matchers.getMainMatcher().test(this._filterData.rawValue))
return true;
...
if(v.type!='user'&&!v.connected_page)
return false;


if(v.disable_autosuggest)
return false;
...
if(this._matchersMode===p.MENTIONS&&
c('MentionsInputMatchers').userMatcher.test(this._filterData.value))
...
this._typeahead.getData().getTextToIndex(v));};
...
_addToken=function(v){'use strict';
var w=v.uid;
if(!Object.prototype.hasOwnProperty.call(this._mentioned,w)){
this._mentioned[w]=v;
...
_removeToken=function(v){'use strict';
if(Object.prototype.hasOwnProperty.call(this._mentioned,v)){
...
this.inform('valueUpdate',{value:w,hasAuxContent:x,textWithMentions:y});};
u.prototype.

_updateMentions=function(){'use strict';
this._offsets=[];
...
_renderHashtags=function(v){'use strict';

this._initHashtagParser(v);
if(!this._hasHashtags||!this._hashtagParser)
return c('htmlize')(v);
...
w.push
(c('htmlize')(v.substring(x,z.rawOffset)),
'<b>',
z.marker,
z.tag,
'</b>');
...
updateValue=function(){'use strict';
...
ea=ea.replace(o,function(ha){return '\\'+ha;});
...
_updateHeight=function(){'use strict';
...
_updateTypeahead=function(){'use strict';
...
setPlaceholder=function(v){'use strict';
this._placeholder=v instanceof c('FbtResult')?
v.toString():v;
...
_updateHighlighter=function(){'use strict';
if(this._highlighterContent)
c('CSS').conditionShow
(this._highlighterContent,
this._numMentioned>0||this.hasAuxContent()||this._hashtags.length);};
...
setAuxContent=function(v){'use strict';
if(this._highlighterContent){
if(!this._highlighterAuxContent){
...
if(v){
c('Input').setPlaceholder(this._input,'');}else 
...
addMention=function(v,w){'use strict';
w=
typeof w==='undefined'?true:w;
var x=w===false?'':' ',
...
getMentions=function(){'use strict';
...
bootloadHashtagParser=function(){'use strict';
if(!this._hashtagParser)
c('Bootloader').loadModules(["HashtagParser"],function(v){
...
getInstance=function(v){'use strict';
...
_checkShouldSwapDataSource=function(v){'use strict';
...
_initHashtagParser=function(v){'use strict';
if(this._hashtagParser)
return;

if(this._matchers.getHashtagMatcher().exec(v))
...
/** js/ui/core/ui/form/mentions/MentionsTypeaheadAreaView.js */




__d('MentionsTypeaheadAreaView',['ContextualTypeaheadView','FamilyRelationshipMentions','Parent'],(function a(b,c,d,e,f,g){var h,i;h=babelHelpers.inherits





(j,c('ContextualTypeaheadView'));i=h&&h.prototype;j.prototype.
getContext=function(){'use strict';
return c('Parent').byClass(this.element,'uiMentionsInput');};
j.prototype.

shouldValidateTypeaheadSelection=function(k){'use strict';
return c('FamilyRelationshipMentions').shouldShowNUX(k);};
j.prototype.

validateTypeaheadSelection=function(k,l){'use strict';
...
/** js/ui/core/ui/form/mentions/mentions-input.js */


__d('legacy:MentionsInput',['MentionsInput'],(function a(b,c,d,e,f,g){

b.MentionsInput=c('MentionsInput');}),3);

/** js/ui/core/ui/typeahead/TypeaheadAreaCore.js */
...
__d('TypeaheadAreaCore',['InputSelection','TextAreaControl','TypeaheadCore','emptyFunction'],(function a(b,c,d,e,f,g){var h,i;h=babelHelpers.inherits
...
(j,c('TypeaheadCore'));i=h&&h.prototype;
...
expandBounds=function(k,l){'use strict';
k=k.toLowerCase();
l=l.toLowerCase();
var m,n,o,p,
q=/\s/;
...
/** js/ui/core/ui/typeahead/behaviors/TypeaheadHoistFriends.js */
...
/** js/ui/core/ui/typeahead/behaviors/LegacyHoistFriendsTypeaheadBehavior.js */
...
/** js/ui/core/ui/typeahead/metrics/TypeaheadMetrics.js */
...
_initImpl=function(j){'use strict';
this.core=j.getCore();
this.view=j.getView();
this.data=j.getData();
...
_logEvent=function(j,k){'use strict';
var l={type:j,data:k,time:Date.now()};
this.log.push(l);};
i.prototype.
...
_setSid=function(j){'use strict';
this.sid=j;




if(typeof this.data.queryData==='object'&&
this.data.queryData!==null){
this.data.queryData.sid=this.sid;}else 

this.data.setQueryData({sid:this.sid});


if(typeof this.data.bootstrapData==='object'&&
this.data.bootstrapData!==null){
this.data.bootstrapData.sid=this.sid;}else 
...
resetWithData=function(j){'use strict';
this.init=c('emptyFunction');
this._initImpl(j);};
i.prototype.
...
resetWithDataBeforeSessionEnd=function(j){'use strict';
...
recordSelect=function(j){'use strict';
var k=j.selected;
if(k.uid==null){
this.recordStat('selected_id','SELECT_NULL');}else 

this.recordStat('selected_id',k.uid);
...
bindSessionStart=function(j,event,k){'use strict';
if(k)
...
bindSessionEnd=function(j,event,k){'use strict';
if(k)
...
dataSubscribe=function(j,k){'use strict';
var l=this.data,
...
this._dataSubscriptions.forEach(function(j){j();});
...
this._logEvent('user_typed',{query:k.value});
if(!k.value)return;
this.query=k.value;
...
k.fetch_context.queryBegin=Date.now();
...
if('filtered_count' in k.response.payload)
this.recordStat('filtered_count',
k.response.payload.filtered_count);

this.recordAvgStat('avg_query_latency',
Date.now()-k.fetch_context.queryBegin);}}.
...
m=this.data.findBestPreviousQuery
(l,
this.data.getQueryIDs())||
'',
...
_sessionStart=function(){'use strict';
if(this.sessionActive)return;
this.sessionActive=true;};
i.prototype.

_sessionEnd=function(){'use strict';
if(!this.sessionActive)return;
this.sessionActive=false;
...
recordStat=function(j,k){'use strict';
this.stats[j]=k;};
i.prototype.
...
recordCountStat=function(j){'use strict';
var k=this.stats[j];
...
recordAvgStat=function(j,k){'use strict';
if(this.avgStats[j]){
this.avgStats[j][0]+=k;
++this.avgStats[j][1];}else 
...
var m=c('Event').listen(j,'focus',function(){
k.init(l);
m.remove();});};
...
/** www/__virtual__/x/XBirthdayMessagePostController.js */
