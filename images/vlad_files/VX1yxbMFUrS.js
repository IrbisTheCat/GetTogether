if (self.CavalryLogger) { CavalryLogger.start_js(["VE3CK"]); }

/** js/ui/core/ui/typeahead/BucketedTypeaheadView.js */





__d('BucketedTypeaheadView',['fbt','DOM','TypeaheadView'],(function a(b,c,d,e,f,g,h){var i,j;i=babelHelpers.inherits
...
(k,c('TypeaheadView'));j=i&&i.prototype;k.prototype.
render=function(l,m,n,o){'use strict';
...
while(l>=0&&l<this.items.length&&
!this.isHighlightable(this.results[l])){
...
buildBuckets=function(l,m){'use strict';
if(!this.typeObjects||!m||!m.length)
return m;
...
buildNoResultsEntry=function(){'use strict';

return {uid:'disabled_result',
type:'disabled_result',
text:h._(["No Results","e00b8f66e40b1b6940f613002799b0b6"])};};

k.prototype.

buildBucketHeader=function(l){'use strict';
var m=this.typeObjects[l];
if(m===undefined)
throw new Error
(l+
" is undefined in "+JSON.stringify(this.typeObjects));
...
if(this.typeObjects){
return c('DOM').create('div',{className:'bucketed'},[m]);}else 

return m;};

k.prototype.

isHighlightable=function(l){'use strict';
return l.type!='header'&&l.type!='disabled_result';};
k.prototype.

select=function(l){'use strict';
var m=this.results[this.index];
if(m&&this.isHighlightable(m))
j.select.call(this,l);};

k.prototype.

updateResults=function(l){'use strict';
this.results=l;};
k.prototype.
...
normalizeIndex=function(l){'use strict';
...
getDefaultIndex=function(l){'use strict';
...
var n=0;
while(!this.isHighlightable(l)&&n<l.length)
n++;
...
__d('ContextualTypeaheadView',['BucketedTypeaheadView','CSS','ContextualLayer','ContextualLayerAutoFlip','ContextualLayerHideOnScroll','DOM','DOMDimensions','Style'],(function a(b,c,d,e,f,g){var h,i;h=babelHelpers.inherits
...
(j,c('BucketedTypeaheadView'));i=h&&h.prototype;j.prototype.
...
initializeLayer=function(){'use strict';
...
if(c('Style').isFixed(this.context)||this.autoflip)
...
if(this.minWidth){
c('Style').set(this.wrapper,'min-width',this.minWidth+'px');}else
if(this.width){
c('Style').set(this.wrapper,'width',this.width+'px');}else 

c('Style').set(this.wrapper,'width',
c('DOMDimensions').getElementDimensions(this.context).width+'px');
...
renderLayer=function(){'use strict';
if(!this.isVisible())
return;
...
/** js/ui/core/ui/form/modules/StickyPlaceholderInput.js */
...
if(c('DOM').isNodeOfType(m,'textarea'))
c('CSS').addClass(p,'uiStickyPlaceholderTextarea');
...
var o=h(m);
if(!o){
c('Input').setPlaceholder(m,n);}else{
...
var o=i(n);
return o&&c('getElementText')(o);}else 
...
__d('TypeaheadCore',['Arbiter','ArbiterMixin','CSS','DOM','Event','Focus','Input','InputSelection','Keys','StickyPlaceholderInput','UserAgent','bind','emptyFunction','mixin'],(function a(b,c,d,e,f,g){var h,i;h=babelHelpers.inherits
...
i.constructor.call(this);this.events=['blur','focus','click','unselect','loading','change'];this.keepFocused=true;this.preventFocusChangeOnTab=false;this.queryTimeout=250;this.resetOnKeyup=true;this.resetOnSelect=false;this.setValueOnSelect=false;
...
init=function(k,l,m){'use strict';
...
if(this.setValueOnSelect&&c('CSS').hasClass(this.inputWrap,'selected'))
...
initData=function(){'use strict';
this.data.subscribe('notify',function(k,l){
if(this.root.id==l.rootid&&!this.element.disabled&&
...
(this.element.getAttribute('singlestate')!=='true'||
l.nullState))
...
c('Event').listen(this.element,{input:k.keyup});
...
initToggle=function(){'use strict';
...
viewMousedown=function(){'use strict';
this.selecting=true;};
j.prototype.
...
viewMouseup=function(){'use strict';
...
keydown=function(event){'use strict';
...
default:setTimeout(this.checkValue.bind(this),0);
return;}
...
keypress=function(event){'use strict';
...
handleTab=function(event){'use strict';
...
afterSelect=function(){'use strict';
...
setEnabled=function(k){'use strict';
var l=k===false;
...
setHiddenValue=function(k){'use strict';
if(typeof k==='number')
k=String(k);
...
getHiddenValue=function(){'use strict';
...
if(this.selectedText&&this.selectedText!=k)
this.unselect();
...
performQuery=function(k){'use strict';
...
this.data.query(this.nextQuery,true,this._exclusions,k);}else{
...
/** js/ui/core/ui/typeahead/behaviors/TypeaheadShowLoadingIndicator.js */





__d('TypeaheadShowLoadingIndicator',['CSS'],(function a(b,c,d,e,f,g){
...
this._subscription=
this._typeahead.subscribe('loading',function(i,j){
c('CSS').conditionClass
...
/** js/ui/core/ui/typeahead/behaviors/LegacyShowLoadingIndicatorTypeaheadBehavior.js */
...
b.TypeaheadBehaviors.showLoadingIndicator=function(h){
h.enableBehavior(c('TypeaheadShowLoadingIndicator'));};}),
3);

/** js/ui/core/ui/typeahead/behaviors/TypeaheadPreventSubmitOnEnter.js */
...
if(c('Event').getKeyCode(j)==c('Keys').RETURN)
j.kill();});};
...
/** js/ui/core/ui/typeahead/behaviors/TypeaheadShowResultsOnFocus.js */
...
this._subscription=
this._typeahead.subscribe('bootstrap',function(i,j){
this.firstFetch
(j,
this._typeahead.getCore(),
this._typeahead.getData());}.
...
this._keyUpListener=
c('Event').listen
(this._typeahead.getCore().getElement(),
'keyup',
function(event){
if(c('Event').getKeyCode(event)==c('Keys').BACKSPACE||
c('Event').getKeyCode(event)==c('Keys').DELETE)
...
firstFetch=function(i,j,k){'use strict';
...
respond=function(i,j){'use strict';
if(!i.getValue())
this.refreshAndShowNeededResults(i,j);};
...
refreshAndShowNeededResults=function(i,j){'use strict';
...
getUidsFromData=function(i){'use strict';
...
k.sort(function(m,n){return m.index-n.index;});
return k.map(function(m){return m.uid;});};
...
/** js/ui/core/ui/typeahead/behaviors/TypeaheadSubmitOnSelect.js */





__d('TypeaheadSubmitOnSelect',['FormSubmit'],(function a(b,c,d,e,f,g){
...
enable=function(){'use strict';
this._subscription=this._typeahead.subscribe('select',function(){
var i=this._typeahead.getCore().getElement().form;
if(i)
i.getAttribute('rel')=='async'?
c('FormSubmit').send(i):i.submit();}.
