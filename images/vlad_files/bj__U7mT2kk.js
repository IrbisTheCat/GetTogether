...
/** js/cavalry/CavalryLoggerImpl.js */
...
if(this.instrumentation_started)
return;
...
if(b.bigPipe&&n.lid===b.bigPipe.lid)
n.
addPiggyback
('cjs_factory_count_e2e',
c('__getTotalFactories')()).

addPiggyback
('cjs_compile_time_e2e',
c('__getCompileTime')()).

addPiggyback
('cjs_factory_time_e2e',
c('__getFactoryTime')());});
...
if(p.metrics)
for(var q in p.metrics)
o.addPiggyback(q,p.metrics[q]);


if(p.usageSnapshot)
o.ttiUsageSnapshot=p.usageSnapshot;
...
if(p.lid===o.lid){
if(p.usageSnapshot)
o.ddUsageSnapshot=p.usageSnapshot;
...
function(event,p){
if(p.lid===o.lid)
o.setAbsTimeStamp('t_marker_bigpipe_e2e',p.ts,true);});},
...
if(p.event===c('PageletEventConstsJS').ARRIVE_END)
o.events[q].phase=p.phase;}});}},
...
if(this.tti_event&&n==this.tti_event)
this.measurePageLoad(o);
...
if(c('ISB').token)
n.fb_isb=c('ISB').token;



for(var o=0;o<k.length;o++)
n[k[o]]=this.values[k[o]];
...
this.addPiggyback
('pagelet_events',
c('PageletEventsHelper').getMetrics(this.lid));


this.setValue
('client_pixel_ratio_10x',
parseInt((window.devicePixelRatio||1)*10,10));
...
if(p.getCodeUsage&&i){
if(!this.start||!this.e2e||!this.dd)
throw new Error('Timestamps are missing in Cavalry. Please report '+
'this to the Web Speed team');


var q=i.findUsedCSSSelectors
(document,
i.getDocumentStylesheets(document)),
...
if(h)
r.html=JSON.stringify
(h.getKillabyteHTMLData());




var s=babelHelpers['extends']({},
i.getDataForSnapshot(this.ddUsageSnapshot),
...
if(!this.tti)
throw new Error
('tti timestamp should always be present if we have usage data');



t.tti=babelHelpers['extends']({},
i.getDataForSnapshot(this.ttiUsageSnapshot),
{bootloads:JSON.stringify(this.getBootloadsUntil(this.tti))});}



c('Arbiter').inform
('cavalry_usage_data_collected',
...
(this.piggy_values.resource_timing_bootloader,
...
return Object.entries(c('Bootloader').getBootloadedComponents()).
filter(function(o){var p=o[0],q=o[1];return q>=this.start&&q<=n;}.bind(this)).
map(function(o){var p=o[0],q=o[1];return p;});},
...
if(o){
this.setAbsTimeStamp('t_tti',o,n);}else 

this.setTimeStamp('t_tti',n);
...
this.addPiggyback
('resource_timing_bootloader',
c('ResourceTimingBootloaderHelper').getMetrics(0,false));
...
var q=
c('NavigationTimingHelper').getAsyncRequestTimings(o);
...
this.setAbsTimeStamp
('t_marker_bigpipe_e2e_with_images',
r.E2E);},
...
var s=q[r];
if(o[s])
this.setAbsTimeStamp(n+s,o[s]+p,true);}}});
...
if('t_tti' in o.values)
o.measurePageLoad(false,o.values.t_tti);}}});
