if (self.CavalryLogger) { CavalryLogger.start_js(["031qe"]); }

/** __static_js_modules__/syncrequeststatusenum.js */
...
__d('BusinessURI.brands',['BizSiteIdentifier.brands','BusinessConf','isFacebookURI','URI'],(function a(b,c,d,e,f,g){var h,i;h=babelHelpers.inherits
...
(j,c('URI'));i=h&&h.prototype;
function j(l,m){'use strict';
i.constructor.call(this,l);
if(c('BizSiteIdentifier.brands').isBizSite()){
var n=m||c('BizSiteIdentifier.brands').getBusinessID();
if(n)
this.addQueryData(c('BusinessConf').BIZ_ID_PARAM_NAME,n);

if(!this.$BusinessURI_isSubdomainWhitelisted(this.getSubdomain()))
this.setSubdomain(c('BusinessConf').DOMAIN);}



if(!c('isFacebookURI')(this))
throw new Error('Business URI must be FB URI');
...
$BusinessURI_isSubdomainWhitelisted=function(l){'use strict';
return l==='developers';};
...
/** js/components/contrib/FBProfilePhotoShadow/FBProfilePhotoShadow.react.js */





__d('FBProfilePhotoShadow.react',['cx','React','joinClasses'],(function a(b,c,d,e,f,g,h){var i,j;i=babelHelpers.inherits
...
render=function(){'use strict';var l=
this.props,m=l.className,n=l.children,o=babelHelpers.objectWithoutProperties(l,['className','children']),
...
function l(){i.apply(this,arguments);}l.propTypes={onClick:k.func.isRequired};


f.exports=l;}),null);

/** js/home/tickerPhoteSnowLiftOpenStatus.js */
...
function Dialog(model){'use strict';
...
showLoading=function(){'use strict';
this._loading=true;
...
getButtonElement=function(button){'use strict';
...
name_filter=function name_filter(elem){return elem.name==button.name;};
...
getContentNode=function(){'use strict';
return require('DOM').find(this._content,'div.dialog_content');};
Dialog.prototype.

getFormData=function(){'use strict';
...
setAllowCrossQuicklingNavigation=function(allow){'use strict';
...
setShowing=function(){'use strict';
this.show();
return this;};
Dialog.prototype.
...
setTitle=function(title){'use strict';
...
setBody=function(body){'use strict';
...
setExtraData=function(data){'use strict';
...
setReturnData=function(data){'use strict';
...
setShowLoading=function(show){'use strict';
...
setFullBleed=function(is_full_bleed){'use strict';
...
setCausalElement=function(elem){'use strict';
...
setUserData=function(data){'use strict';
...
getUserData=function(){'use strict';
...
setAutohide=function(autohide){'use strict';
if(autohide){
if(this._showing){
this._autohide_timeout=setTimeout(require('shield')(this.hide,this),autohide);}else 

this._autohide=autohide;}else{
...
setSummary=function(summary){'use strict';
...
setButtons=function(b){'use strict';
var buttons,button;
...
if(!Array.isArray(b)){
buttons=Array.from(arguments);}else 
...
var button_input=require('DOM').create
('input',
{type:'button',
name:button.name||'',
value:button.label}),
...
button_node=require('DOM').create
('label',
...
if(require('CSS').hasClass(button_node,'uiButtonSpecial'))
require('CSS').removeClass(button_node,'uiButtonConfirm');}
...
require('DOM').prependContent
(button_node,
require('DOM').create('img',{src:button.icon,className:'img mrs'}));



if(button.disabled)
require('Button').setEnabled(button_node,false);


require('Event').listen
(button_input,
'click',
...
if(attr.indexOf('data-')===0&&attr.length>5)
...
setButtonsMessage=function(message){'use strict';
...
_updateButtonVisibility=function(){'use strict';
...
setClickButtonOnEnter=function(input_id,button){'use strict';
...
require('Event').listen(this._nodes.body,'keypress',function(event){
...
setStackable=function(stackable,shown){'use strict';
...
setHandler=function(handler){'use strict';
...
setCancelHandler=function(cancelHandler){'use strict';
...
setCloseHandler=function(close_handler){'use strict';
...
clearHandler=function(){'use strict';
...
setPostURI=function(post_uri,asynchronous){'use strict';
if(asynchronous===undefined)
asynchronous=true;


if(asynchronous){
this.setHandler(this._submitForm.bind(this,'POST',post_uri));}else 
...
setGetURI=function(get_uri){'use strict';
...
setModal=function(modal){'use strict';
...
setSemiModal=function(clickout){'use strict';
...
if(!require('DOM').contains(this._popup,e.getTarget()))
this.hide();}.
...
setWideDialog=function(is_wide){'use strict';
...
setContentWidth=function(width){'use strict';
...
setTitleLoading=function(loading){'use strict';
if(loading===undefined)
loading=true;

var header=require('DOM').find(this._popup,'h2.dialog_title');
if(header)
require('CSS').conditionClass(header,'loading',loading);
...
setSecure=function(is_secure){'use strict';
...
setClassName=function(class_name){'use strict';
...
setFadeEnabled=function(enabled){'use strict';
...
setFooter=function(footer){'use strict';
...
setAutoFocus=function(focus){'use strict';
...
setTop=function(top){'use strict';
this._top=top;
...
onloadRegister=function(handler){'use strict';
...
setAsyncURL=function(url){'use strict';
...
setAsync=function(async_request){'use strict';
...
this.setBody(dialog);}else 

this._setFromModel(dialog);
...
orig_error_handler=
request.getErrorHandler()||require('emptyFunction'),
orig_trans_error_handler=
request.getTransportErrorHandler()||require('emptyFunction'),
...
_format=function(content){'use strict';




if(typeof content=='string'){
content=require('HTML')(content);}else 

content=require('HTML').replaceJSONWrapper(content);

if(content instanceof require('HTML'))
content.setDeferred(true);

return content;};
Dialog.prototype.

_update=function(){'use strict';
if(!this._showing)
return;
...
this._autohide_timeout=setTimeout(require('bind')(this,'hide'),this._autohide);
...
_runOnloads=function(){'use strict';
...
try{this._onload_handlers[i]();}catch(
ex){}
...
dialog_width+=2*Dialog._PADDING_WIDTH;}else

if(this._wide_dialog){
dialog_width+=Dialog.SIZE.WIDE;}else 
...
_updateZIndex=function(){'use strict';
...
_renderDialog=function(){'use strict';
this._updateZIndex();
this._pushOntoStack();
...
var input=require('Form').getFirstElement(this._content,
['input[type="text"]',
'textarea',
'input[type="password"]']);

if(input){
setTimeout(require('Form').focusFirst.bind(this,this._content),0);}else 
...
var bottom=require('Vector').getElementDimensions(this._content).y+
require('Vector').getElementPosition(this._content).y;
...
_buildDialog=function(){'use strict';
...
if(!this._popup)
this._popup=require('DOM').create('div',{className:'generic_dialog_popup'});

this._obj.appendChild(this._popup);

require('CSS').addClass(this._obj,'pop_dialog');
if(require('Locale').isRTL())
require('CSS').addClass(this._obj,'pop_dialog_rtl');


require('DOM').setContent(this._popup,
require('DOM').create('div',{className:'pop_container_advanced'},
require('DOM').create('div',{className:'pop_content',id:'pop_content'})));
...
var loading=require('DOM').create
('div',
{className:'dialog_loading'},fbt._(["Loading...","0007976d9b948d4ca9e57de7b555b2c5"])),

title_inner=require('DOM').create('span'),
title=require('DOM').create
('h2',
{className:'dialog_title hidden_elem',
id:'title_'+this._uniqueID},
title_inner),
summary=require('DOM').create('div',{className:'dialog_summary hidden_elem'}),
body=require('DOM').create('div',{className:'dialog_body'}),
buttons=require('DOM').create('div',{className:'rfloat mlm'}),
button_message=require('DOM').create('div',{className:'dialog_buttons_msg'}),
button_wrapper=require('DOM').create
('div',
{className:'dialog_buttons clearfix hidden_elem'},
[buttons,button_message]),
footer=require('DOM').create('div',{className:'dialog_footer hidden_elem'}),
content=require('DOM').create
('div',
...
_activeResize=function(){'use strict';
...
resetDialogPosition=function(){'use strict';
if(!this._popup)
return;
...
_resetDialogObj=function(){'use strict';
...
top=Dialog._PAGE_MARGIN;}else
if(top>empty_vertical_space)


top=Dialog._PAGE_MARGIN+Math.max(empty_vertical_space,0)/2;
...
if(!is_fixed)
top+=require('Vector').getScrollPosition().y;
...
require('CSS').conditionClass
(document.documentElement,
'generic_dialog_overflow_mode',
scroll);};
Dialog.prototype.

_fadeOut=function(temporary){'use strict';
if(!this._popup)
return;


try{new (require('Animation'))(this._obj).
duration(0).
checkpoint().
to('opacity',0).
hide().
...
_hide=function(temporary){'use strict';
if(this._obj)
this._obj.style.display='none';
...
require('DOM').contains(this._obj,document.activeElement))
require('Focus').set(this._previous_focus);
...
_handleButton=function(button){'use strict';
if(typeof button=='string')
button=Dialog._findButton(this._buttons,button);
...
this.cancel();}else
if(Dialog.call_or_eval(this,this._handler,{button:button})!==
false)
this.hide();};

Dialog.prototype.

_submitForm=function(method,uri,button){'use strict';
...
if(this._extra_data)
Object.assign(data,this._extra_data);


var async_request=new (require('AsyncRequest'))().
setURI(uri).
setData(data).
setMethod(method).
...
_setFromModel=function(original_model){'use strict';
var model={};
...
_updateBottom=function(){'use strict';
var bottom=require('Vector').getElementDimensions(this._content).y+
require('Vector').getElementPosition(this._content).y;
...
_pushOntoStack=function(){'use strict';
var stack=Dialog._stack;

if(!stack.length)
require('Arbiter').inform('layer_shown',{type:'Dialog'});
...
prev_dialog._hide();}else
if(!prev_dialog._shown_while_stacked)
prev_dialog._hide(true);}};


Dialog.prototype.

_popFromStack=function(){'use strict';
var stack=Dialog._stack,
...
if(was_top)
stack[stack.length-1].show();}else 


require('Arbiter').inform('layer_hidden',{type:'Dialog'});};
...
_updateMaxBottom=function(){'use strict';
Dialog.max_bottom=
Math.max.apply(Math,Dialog._bottoms);};
Dialog.
...
newButton=function(name,label,className,handler){'use strict';
var button={name:name,label:label};
if(className)
...
getCurrent=function(){'use strict';
var stack=Dialog._stack;
return stack.length?stack[stack.length-1]:null;};
Dialog.


hideCurrent=function(){'use strict';
...
bootstrap=function(uri,data,read_only,method,model,elem){'use strict';
data=data||{};
...
var request=new (require('AsyncRequest'))().
...
showFromModel=function(model,causal_element){'use strict';
...
_findButton=function(buttons,name){'use strict';
if(buttons)
for(var i=0;i<buttons.length;++i)
...
_tearDown=function(is_page_transition){'use strict';
var stack=Dialog._stack.slice();
for(var ii=stack.length-1;ii>=0;ii--)
if(is_page_transition&&!stack[ii]._cross_transition||
...
return true;}else
if(cancel_button){
button_to_simulate=cancel_button;}else
if(buttons.length==1){
button_to_simulate=buttons[0];}else 
...
call_or_eval=function(obj,func,args){'use strict';
if(!func)
return undefined;
...
{OK:
{name:'ok',
label:fbt._(["OK","ea40ab962606886b4ad0c9e7bba7dcf3"])},


CANCEL:
{name:'cancel',
label:fbt._(["Cancel","47519dd63967d2eefa17a94acc8182e0"]),
className:'inputaux'},


CLOSE:
{name:'close',
label:fbt._(["Close","b74978868bfd773a3f54dbb806d0fef3"])},


NEXT:
{name:'next',
label:fbt._(["Next","8085eb3dd789744b28ffbce02c99580d"])},


SAVE:
{name:'save',
label:fbt._(["Save","fe72dd48dbfcd5e34f3e239cad46b860"])},


SUBMIT:
{name:'submit',
label:fbt._(["Submit","e7c1dba713bf1e90f342d292ce19bc91"])},


CONFIRM:
{name:'confirm',
label:fbt._(["Confirm","b852fce971c4121283ab9832e816374d"])},


DELETE:
{name:'delete',
label:fbt._(["Delete","bdceaadab76e05d13bdeb4418275324f"])},
...
global.Dialog=Dialog;}),null);

/** js/ui/core/xui/ButtonGroupX.js */





__d('ButtonGroupX',['ArbiterMixin','mixin'],(function a(b,c,d,e,f,g){var h,i;h=babelHelpers.inherits
...
initButtonListeners=function(){'use strict';
...
getSelectedValue=function(){'use strict';
return this._selected?this._selected.getValue():null;};
j.prototype.

selectButton=function(k){'use strict';
if(this._selected!==k){
this.setSelected(k);
...
setSelected=function(k){'use strict';
if(this._selected!==k){
if(this._selected)
...
setSelectedValue=function(k){'use strict';
